...
%% script for learning albedo-scale network
%% written by Seungryong Kim, Yonsei University, Seoul, Korea
function main_albedo_gradient_scale(varargin)
    ...
    trainOpts.expDir = 'data/albedo-gradient-scale-experiment';
...
% --------------------------------------------------------------------
    for i = 1:length(batch) 
        ...
        load(fullfile(imdb.images.data{batch(i)},'im_label2.mat'));
        ...
        label(:,:,:,i) = cat(3,single(im_label2),single(albedo_gradient));
    end
