...
%% script for learning depth-scale network
%% written by Seungryong Kim, Yonsei University, Seoul, Korea
function main_depth_gradient_scale(varargin)
    ...
    trainOpts.expDir = 'data/depth-gradient-scale-experiment';
...
% --------------------------------------------------------------------
    for i = 1:length(batch) 
        ...
        load(fullfile(imdb.images.data{batch(i)},'im_label1.mat'));
        ...
        label(:,:,:,i) = cat(3,single(im_label1),single(depth_gradient));
    end
